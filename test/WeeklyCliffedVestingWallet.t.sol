...
import "../src/WeeklyCliffedVestingWallet.sol";
...
contract ContinuousCliffedVestingWalletTest is Test {
    ...
    function createWallet(address beneficiary, uint64 start, uint64 cliff, uint64 duration) internal returns (DiscreteCliffedVestingWallet) {
        return DiscreteCliffedVestingWallet(payable(new WeeklyCliffedVestingWallet(beneficiary, start, duration, cliff)));
    }
    ...
    function testExecuteVestingSchedule(address beneficiary, uint64 start, uint64 cliff, uint64 duration, uint256 amount) public {
        ...
        for (uint i=subOrZero(start, durationSeconds); i<start+2*durationSeconds; i+=durationSeconds/30+1) {
            ...
            } else if (i < start+durationSeconds) {
                uint256 expected = (i - start) / 604800  * amount / (duration );
